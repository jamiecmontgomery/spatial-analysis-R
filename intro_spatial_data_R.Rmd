...
cc = readOGR(dsn='data',layer='ca_current',verbose=FALSE)
```
...
```{r calc sst}
sstAvg = calc(sstStack,fun=function(x){mean(x,na.rm=T)-273.15}) #calculate the average SST in Kelvin then convert to Celsius
...
Read in this data the same way as the SST data, using `raster()`. This data is the &#937; aragonite saturation state of the water. For a frame of reference, values at or less than one are undersaturated in regards to aragonite and pose large problems for calcifying organisms.  
```{r avg oa}
...
oa_proj = projectRaster(oaAvg,crs = ('+proj=longlat'));plot(oa_proj,col=cols) #see a warning about finite points
...
oa_res = resample(oa_crop,sstAvg,method='ngb',progress='text',verbose=F)
oa_bil = resample(oa_crop,sstAvg,method='bilinear',progress='text',verbose=F)
...
oa = projectRaster(oaAvg,crs=('+proj=longlat'))%>%
        crop(.,sstAvg)%>%
          resample(.,sstAvg,method='ngb',progress='text',verbose=F);plot(oa,col=cols)
```
...
Setting our parameters for Aquaculture suitability

These are completely arbitrary and are just to show the utility of the following functions. 

Let's say our species grow best in waters that are between 12 and 18 degrees celsius
...
plot(sstPref,col=cols)
plot(cc,add=T)
```

Our species also prefers water that has an aragonite saturation state between 2.7 and 3

```{r}

oaPref = oa
oaPref[oaPref<2.4|oaPref>3]<-NA
plot(oaPref,col=cols)
plot(cc,add=T)
```
...
Set all viable cells to equal 1
```{r}

sstBin = sstPref
sstBin[!is.na(sstBin)]<-1

oaBin = oaPref
oaBin[!is.na(oaBin)]<-1
```

Add the two rasters together. Cells where both enviromental requirements are met will be equal to 2.
```{r}
combined = sum(sstBin,oaBin,na.rm=T);combined
plot(combined);plot(cc,add=T)
```


Select just viable cells
```{r}

prefRas = calc(combined,fun=function(x){ifelse(x==2,1,NA)})

plot(prefRas);plot(cc,add=T)
```

This could be all you need - but a lot more can be done.
...
plot(m);plot(cc,add=T)
```

Crop to the suitable region
```{r}

c = crop(m,extent(-125,-110,25,35)) #setting extent by eyeing it
```

Another nifty thing - draw extent
```{r}
...
#ymin        : 27.48709 
#ymax        : 35.00825 
ext=extent(-126.729,-113.3571,27.48709,35.00825)
c=crop(m,ext)
plot(c);plot(cc,add=T)
```


Add in Marine Protected areas and crop to our area
```{r mpas}

#marine protected areas
mpa = readOGR(dsn='data',layer='MPA_State');mpa
plot(mpa)

# try to crop to our extent
mpa_c = crop(mpa,ext);mpa_c

#notice mpa_c does not throw an error, but it is a null object. This is because the MPA shapefile is not in the same projection

mpa = spTransform(mpa,CRS("+proj=longlat"));mpa #spTransform is part of rgdal package
...
nms = readOGR(dsn='data',layer='National Marine Sanctuaries')%>%
          spTransform(.,CRS("+proj=longlat"));nms
...
Summary stats - looking at total area in Mexico and California for aquaculture
...
cc_ras = rasterize(cc,sstAvg,progress='text');cc_ras
plot(cc_ras) #R automatically assigned 1 and 2 as the ID which is the value of allcells in those regions. You can also pick your own

cc_ras_ID = rasterize(cc,sstAvg,progress='text',field="ID");cc_ras_ID
plot(cc_ras_ID)
...
plot(prefRas)
cells = zonal(prefRas,cc_ras,fun='sum') #total number of cells since they are all equal to 1

#total area (bring in the area function)

prefArea = area(prefRas,na.rm=T);plot(prefArea,col=cols) #gives km2

area = zonal(prefArea,cc_ras,fun='sum')%>%as.data.frame()
```


plot(c)

k = mask(c,mpa,inverse=T)
k = mask(k,nms,inverse=T)

...
system.time(ak_shp_sf <- st_read(dsn = "shapefiles", layer="ak_regions")) 
...
Next we compute the total population for each region and drop the geometry because we are only looking for the data without a geospatial reference at this point. We drop the geometry using `as.data.frame`, on the advice of the `sf::tidyverse` help page. We will join this data to the AK regions shapefile next.
